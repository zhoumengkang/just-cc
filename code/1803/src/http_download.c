...
static bool is_https_request = false;
static SSL_CTX *ctx;
static SSL *ssl;
...
void https_connect_init(int fd)
{
...
int http_connect(http_url * url)
{
	...
	if (strcmp(url->scheme, "https") == 0) {
		...
		https_connect_init(sockfd);
	}

	return sockfd;
}

int http_send_request(int fd, http_url * url)
{
	int len = 0;
	char *http_request = http_create_request(url);

	if (is_https_request) {
		len = SSL_write(ssl, http_request, strlen(http_request));
	} else {
...
int http_download(int sockfd, const char *filename)
{
	...
	while ((len = is_https_request ?
		SSL_read(ssl, response_buf, buf_size) : read(sockfd,
							     response_buf,
							     buf_size)) > 0) {
